...
export class Transforms_Sandbox extends Transforms_Sandbox_Base {
    ...
    display(context, program_state) {
        ...
        const blue = hex_color("#1a9ffa"), yellow = hex_color("#fdc03a"), green = color(0, 1, 0, 1), orange = hex_color("#ff9500") 
        // Variable model_transform will be a local matrix value that helps us position shapes.
        // It starts over as the identity every single frame - coordinate axes at the origin.

        // model_transform is a matrix that represents the cumulative transform to be applied to the center point of our shape
        // We see in later lines that we pass this transformation matrix into the draw function for a given shape
        let model_transform = Mat4.identity();

        // NOTE: The later in the program that we add a transformation to the model_transform, the "earlier" it gets
        // applied to the initial point. For example, if we did model_transform.times(translation) and then model_transform(rotation),
        // the resulting model_transform would describe performing the rotation on the shape first, followed by the translation
        ...
        // Draw a green sphere on the right
        let model_transform_sib_right = model_transform.times(Mat4.translation(2, 0, 0));
        this.shapes.ball.draw(context, program_state, model_transform_sib_right, this.materials.metal.override(green));
        
        
        // Draw an orange sphere on the left
        let model_transform_sib_left = model_transform.times(Mat4.translation(-2, 0, 0));
        this.shapes.ball.draw(context, program_state, model_transform_sib_left, this.materials.metal.override(orange));
        ...
        // Spin a cube that appears to be attached to the green sphere at an angle
        model_transform_sib_right = model_transform_sib_right.times(Mat4.rotation(-0.6, 0, 0, 1))
                .times(Mat4.translation(2, 0, 0))
                .times(Mat4.rotation(2*t, 1, 0, 0));
        this.shapes.box.draw(context, program_state, model_transform_sib_right, this.materials.plastic.override(yellow));
